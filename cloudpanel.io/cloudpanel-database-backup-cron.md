...
## Observed State — Backup Coverage

Before the architecture, three facts about **this specific server**, noted while documenting
it. Recorded as findings, not as a call to action:
...
This is worth understanding as an *architectural* point rather than a to-do: it is the
default outcome of CloudPanel's design, not a misconfiguration here. None of the three
backup systems self-enables, `remote-backup:create` exits 0 when unconfigured, and the
command classes hardcode success. A stock install looks healthy while backing up nothing.
That combination is the single most important lesson in this document for anyone building
their own panel — see [§6.4](#64-cross-cutting-make-failures-loud).

[§5](#5-operational-recipes) records the commands that would change this, for reference.
...
## 5. Operational Recipes *(reference)*

### 5.1 What enabling backups would involve

Recorded for reference — two steps would close the gap described above.
